...
-- Tiempo de generación: 13-05-2025 a las 03:02:49
...
-- Procedimientos
--
CREATE DEFINER=`u415516518_agenda_mujeres`@`127.0.0.1` PROCEDURE `ActualizarEstadoPagoPorCedula` (IN `cedulaProfesor` VARCHAR(20))   BEGIN
...
CREATE DEFINER=`u415516518_agenda_mujeres`@`127.0.0.1` PROCEDURE `DetallesPagoCurso` (IN `p_IdCurso` INT)   BEGIN
    SELECT 
        m.Id ,
        c.NombreCurso,
        m.Nombre  ,
...
CREATE DEFINER=`u415516518_agenda_mujeres`@`127.0.0.1` PROCEDURE `EliminarProfesor` (IN `profesorID` INT)   BEGIN
...
CREATE DEFINER=`u415516518_agenda_mujeres`@`127.0.0.1` PROCEDURE `InsertarMovimientoPagoProfesor` (IN `p_tipoMovimiento` TINYINT, IN `p_monto` DECIMAL(10,2), IN `p_idCurso` INT, IN `p_idProfesor` INT, IN `p_Notas` TEXT, IN `p_comprobante` VARCHAR(100), IN `p_fecha` DATETIME, IN `p_calculoPago` INT)   BEGIN
...
CREATE DEFINER=`u415516518_agenda_mujeres`@`127.0.0.1` PROCEDURE `InsertarMovimientoPorMatriculaPagoCliente` (IN `p_IdMatricula` INT, IN `p_Cuota` DECIMAL(10,2), IN `p_Notas` VARCHAR(255), IN `p_Comprobante` VARCHAR(255))   BEGIN
...
CREATE DEFINER=`u415516518_agenda_mujeres`@`127.0.0.1` PROCEDURE `InsertarMovimientoPorMatriculaPagoProfesor` (IN `p_IdCurso` INT, IN `p_Notas` VARCHAR(255), IN `p_Comprobante` VARCHAR(255))   BEGIN
...
CREATE DEFINER=`u415516518_agenda_mujeres`@`127.0.0.1` PROCEDURE `InsertarPagosProfesores` ()   BEGIN
...
CREATE DEFINER=`u415516518_agenda_mujeres`@`127.0.0.1` PROCEDURE `ObtenerHorariosCurso` (IN `cursoID` INT)   BEGIN
    SELECT 
        h.ID AS `idHorario`,
        h.DiaSemana AS `Dia`,
...
CREATE DEFINER=`u415516518_agenda_mujeres`@`127.0.0.1` PROCEDURE `SP_BORRAR_REGISTROS_MATRICULA` (IN `p_idMatricula` INT)   BEGIN
    -- Eliminar de matriculas
    DELETE FROM movimientosdinero WHERE idMatricula = p_idMatricula;
    DELETE FROM matriculas WHERE Id = p_idMatricula;
END$$

CREATE DEFINER=`u415516518_agenda_mujeres`@`127.0.0.1` PROCEDURE `SP_CREAR_USUARIO` (IN `p_email` VARCHAR(255), IN `p_password` VARCHAR(255), IN `p_fullName` VARCHAR(255), IN `p_activo` INT, IN `p_rolAdmin` INT, IN `p_rolServicio` INT, IN `p_rolContable` INT, IN `p_rolReporteria` INT)   BEGIN
    DECLARE userID INT;
...
CREATE DEFINER=`u415516518_agenda_mujeres`@`127.0.0.1` PROCEDURE `SP_EDITAR_USUARIO_V2` (IN `p_usuarioID` INT, IN `p_email` VARCHAR(255), IN `p_password` VARCHAR(255), IN `p_fullName` VARCHAR(255), IN `p_rolAdmin` INT, IN `p_rolServicio` INT, IN `p_rolContable` INT, IN `p_rolReporteria` INT, IN `p_activo` BIT)   BEGIN
...
CREATE DEFINER=`u415516518_agenda_mujeres`@`127.0.0.1` PROCEDURE `SP_ELIMINAR_MATRICULA` (IN `idMatricula` INT)   BEGIN
    DECLARE v_Nombre VARCHAR(255);
    DECLARE v_Cedula VARCHAR(50);
    DECLARE v_Telefono VARCHAR(50);
    DECLARE v_Correo VARCHAR(255);
    DECLARE v_estadoPago VARCHAR(50);
    DECLARE v_comprobante VARCHAR(255);
    DECLARE v_formaPago VARCHAR(100);

    -- Obtener los datos antes de eliminar
    SELECT Nombre, Cedula, Telefono, Correo, estadoPago, comprobante, formaPago
    INTO v_Nombre, v_Cedula, v_Telefono, v_Correo, v_estadoPago, v_comprobante, v_formaPago
    FROM matriculas
    WHERE Id = idMatricula;

    -- Insertar en deserción
    INSERT INTO desercion (Nombre, Cedula, Telefono, Correo, estadoPago, comprobante, formaPago)
    VALUES (v_Nombre, v_Cedula, v_Telefono, v_Correo, v_estadoPago, v_comprobante, v_formaPago);

    -- Eliminar de matriculas
    DELETE FROM matriculas WHERE Id = idMatricula;
END$$

CREATE DEFINER=`u415516518_agenda_mujeres`@`127.0.0.1` PROCEDURE `SP_INSERTAR_MATRICULAS_ADMIN` (IN `p_Nombre` VARCHAR(100), IN `p_Cedula` VARCHAR(50), IN `p_Telefono` VARCHAR(20), IN `p_Correo` VARCHAR(100), IN `p_EstadoPago` VARCHAR(50), IN `p_IdCurso` INT, IN `p_IdHorario` INT)   BEGIN
    DECLARE v_cantidadCuotas INT;
    DECLARE v_fechaInicio DATE;
    DECLARE v_cuotasPendientes INT;
    DECLARE v_diasRestantes INT;
    DECLARE v_mesesTranscurridos INT;

    -- Obtener datos del curso
    SELECT cantidadCuotas, fechaInicio
    INTO v_cantidadCuotas, v_fechaInicio
    FROM cursos
    WHERE ID = p_IdCurso;
    ...
    SET v_mesesTranscurridos = PERIOD_DIFF(DATE_FORMAT(CURDATE(), '%Y%m'), DATE_FORMAT(v_fechaInicio, '%Y%m'));

    -- Calcular días restantes para cumplir el próximo "mes exacto"
    SET v_diasRestantes = DATEDIFF(DATE_ADD(v_fechaInicio, INTERVAL v_mesesTranscurridos + 1 MONTH), CURDATE());

    -- Lógica de cuotas pendientes
    IF v_fechaInicio > CURDATE() THEN
        SET v_cuotasPendientes = v_cantidadCuotas;
    ELSE
        IF v_diasRestantes < 5 THEN
            SET v_cuotasPendientes = GREATEST(v_cantidadCuotas - v_mesesTranscurridos - 1, 0);
        ELSE
            SET v_cuotasPendientes = GREATEST(v_cantidadCuotas - v_mesesTranscurridos, 0);
        END IF;
    END IF;

    -- Insertar en la tabla matriculas
    INSERT INTO matriculas (
        Nombre, Cedula, Telefono, Correo, estadoPago, IdCurso, IdHorario, cuotasPendientes, fechaCreacion
    ) VALUES (
        p_Nombre, p_Cedula, p_Telefono, p_Correo, p_EstadoPago, p_IdCurso, p_IdHorario, v_cuotasPendientes, NOW()
    );
END$$

CREATE DEFINER=`u415516518_agenda_mujeres`@`127.0.0.1` PROCEDURE `SP_INSERTAR_MATRICULAS_USUARIO` (IN `p_Nombre` VARCHAR(100), IN `p_Cedula` VARCHAR(50), IN `p_Telefono` VARCHAR(20), IN `p_Correo` VARCHAR(100), IN `p_EstadoPago` VARCHAR(50), IN `p_Comprobante` VARCHAR(255), IN `p_IdCurso` INT, IN `p_IdHorario` INT)   BEGIN
    DECLARE v_cantidadCuotas INT;
    DECLARE v_fechaInicio DATE;
    DECLARE v_cuotasPendientes INT;
    DECLARE v_diasRestantes INT;
...
CREATE DEFINER=`u415516518_agenda_mujeres`@`127.0.0.1` PROCEDURE `SP_RevertirPago` (IN `p_id_movimiento` INT, IN `p_id_matricula` INT)   BEGIN
...
CREATE DEFINER=`u415516518_agenda_mujeres`@`127.0.0.1` PROCEDURE `ValidarCuotasPagadasEnvioMensaje` (IN `idMatricula` INT)   BEGIN
...
CREATE DEFINER=`u415516518_agenda_mujeres`@`127.0.0.1` PROCEDURE `ValidarYInsertarHorario` (IN `p_DiaSemana` VARCHAR(20), IN `p_HoraInicio` TIME, IN `p_HoraFin` TIME, IN `p_IdCurso` INT, IN `p_IdProfesor` INT, IN `p_Cupo` INT, OUT `p_Resultado` INT, OUT `p_Mensaje` VARCHAR(255))   BEGIN
...
(263, 'Yessenia Chacón Rodriguez ', '205270658', '50689357026', 'ychaconcavalier@gmail.com'),
(264, 'Alba Arce Varela', '401630090', '50688342982', 'albaarce1@hotmail.com'),
(265, 'Tamara Castro Venegas', '118680488', '50686519595', 'tamyg.castro@gmail.com'),
(266, 'Sara campos Rodríguez ', '402730566', '50684597934', 'saracamposrodriguez0880@gmail.com'),
(267, 'Carmen', '203100672', '50687070035', 'carmenemilia56@yahoo.com'),
(268, 'Carolina Gutiérrez Fernández', '401330434', '50683341558', 'carolgdr@hotmail.com'),
(269, 'Carmen Campos Solís ', ' 203100672', '50687070035', 'carmenemilia56@yahoo.com'),
(270, '1231233123123', '12312312312', '50612312312', '123123123123');
...
CREATE TABLE `cursos` (
  ...
  `MatriculasFueraTiempo` tinyint(1) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
(58, 'Carmen', '203100672', '50687070035', 'carmenemilia56@yahoo.com', 'Pendiente', '17512231', NULL),
(59, 'Ana Jane Camacho Soto', '401070669', '50684416868', 'radiobarvacr@gmail.com', 'Pendiente', 'BECADA', NULL);
...
(83, 'Lunes', '18:00:00', '20:00:00', 79, 29, 60);
...
(167, 'Raquel Campos Chavarría', 'Portugués - principiantes (virtual)', 1, 38, '401910222', '63634327', 'reunice.cha@gmail.com', 52500.00, 0),
(168, 'Pablo Álvarez Leitón', 'Barber Shop', 2, 49, '402080280', '70664754', 'pabloa_2690@hotmail.com', 67500.00, 0),
(169, 'Pablo Álvarez Leitón', 'Barber shop - avanzado (solamente alumnos que', 2, 54, '402080280', '70664754', 'pabloa_2690@hotmail.com', 45000.00, 0),
...
(13, 'Carmen Murillo Espinoza', '20482781', '50689873903', 'carmenmurilloespinoza@gmail.com', 'Pendiente', '54713110', 17, 16, NULL, 0, 1, 1, 1, '2025-03-23 16:57:08'),
(14, 'Guadalupe Murillo Espinoza', '2473966', '50685068415', 'murillog972@gmail.com', 'Pendiente', '54713110', 17, 16, NULL, 0, 1, 1, 1, '2025-03-23 16:57:08'),
(20, 'Catalina Sánchez Sánchez ', '206690166', '50683345869', 'cata23ss@hotmail.com', 'Pendiente', '93929170', 43, 48, NULL, 0, 1, 1, 1, '2025-03-23 16:57:08'),
...
(28, 'Milena Mendoza Valenciano', '701100837', '50689932919', 'mmendoza@una.ac.cr', 'Pendiente', '94960341', 44, 49, NULL, 0, 1, 1, 1, '2025-03-23 16:57:08'),
...
(37, 'Valentina gabriela Morales Sánchez ', '402730546', '50684490254', 'valentinagabriela1406@gmail.com', 'Pendiente', '46064421', 40, 45, NULL, 0, 1, 1, 1, '2025-03-23 16:57:08'),
(38, 'Marianela Cerdas Gómez ', '402070781', '50685501226', 'nelac0809@gmail.com', 'Pendiente', '6526881', 16, 15, NULL, 0, 1, 1, 1, '2025-03-23 16:57:08'),
...
(41, 'Miriam Salas Arce ', '108410277', '50672134027', 'mirsalasa@gmail.com', 'Pendiente', '56520979 ', 17, 16, NULL, 0, 1, 1, 1, '2025-03-23 16:57:08'),
(42, 'Viviana Cerdas Gómez ', '207450030', '50689778594', 'vivianacerdasgomez@gmail.com', 'Pendiente', '2025030715283006429537559', 16, 15, NULL, 0, 2, 1, 1, '2025-03-23 16:57:08'),
...
(50, 'Catalina Sánchez Sánchez ', '206690166', '50683345869', 'cata23ss@hotmail.com', 'Pendiente', '93927078', 44, 49, NULL, 0, 1, 1, 1, '2025-03-23 16:57:08'),
(51, 'Krysta Nicole Navarro Carvajal', '117640290', '50687842662', 'nicolnc02@gmail.com', 'Pendiente', '52859018', 44, 49, NULL, 0, 1, 1, 1, '2025-03-23 16:57:08'),
(52, 'Irene Villalobos León', '402110502', '50689158623', 'ire_villalobos_18@hotmail.com', 'Pendiente', '94889657', 44, 49, NULL, 0, 1, 1, 1, '2025-03-23 16:57:08'),
(53, 'Marjorie Bonilla Benavides ', '1-0621-0927 ', '50683496850', 'yuriboni64@gmail.com', 'Pendiente', '366234766', 27, 30, NULL, 0, 1, 1, 1, '2025-03-23 16:57:08'),
(54, 'Elizabeth Vargas López ', '104520944', '50661148230', 'marieliza317@gmail.com', 'Pendiente', '579041865', 27, 30, NULL, 0, 1, 1, 1, '2025-03-23 16:57:08'),
...
(57, 'Paola Vargas', '109600558', '506', 'jpavazu@gmail.com', 'Pendiente', 'BECADA', 17, 16, NULL, 0, 1, 1, 1, '2025-03-23 16:57:08'),
...
(60, 'José Mario Valerio Berrocal ', '401940089', '50684553578', 'jvalerio419@gmail.com', 'Pendiente', '95926675 ', 42, 47, NULL, 0, 1, 1, 1, '2025-03-23 16:57:08'),
...
(62, 'Patricia Ramírez Rimolo ', '108270565 ', '50670395794', 'masiescara98@gmail.com', 'Pendiente', '6495351514', 16, 15, NULL, 0, 1, 1, 1, '2025-03-23 16:57:08'),
...
(64, 'Vanessa Rodríguez Serrano ', '303750678', '50661481859', 'vrs3981@hotmail.com', 'Pendiente', 'BECADA', 44, 49, NULL, 0, 1, 1, 1, '2025-03-23 16:57:08'),
(65, 'Kerlyn Fernández Obando ', '112860687', '50683193219', 'kerlyn_07@yahoo.es', 'Pendiente', '47439969', 23, 25, NULL, 0, 4, 1, 1, '2025-03-23 16:57:08'),
...
(73, 'Valeria campos lobo', '402210923', '50672041477', 'valecl2693@gmail.com', 'Pendiente', '9189733', 43, 48, NULL, 0, 1, 1, 1, '2025-03-23 16:57:08'),
(74, 'Elodia Hernandez Vargas ', '401440460 ', '50687830596', 'elo.her@hotmail.es', 'Pendiente', '6502267521', 45, 50, NULL, 0, 1, 1, 1, '2025-03-23 16:57:08'),
(75, 'Doris Carolina Rodríguez Ramírez', '112780769', '50683467785', 'dorys18@gmail.com', 'Pendiente', '66633261', 23, 25, NULL, 0, 4, 1, 1, '2025-03-23 16:57:08'),
...
(84, 'Mercedes Manzanares Barahona', '800770707', '50687922465', 'william.Bermudez.ramirez@gmail.com', 'Pendiente', '92865631', 43, 48, NULL, 0, 1, 1, 1, '2025-03-23 16:57:08'),
...
(87, 'Marycruz Ocampo Aguilar ', '108080245', '50688388581', 'maricruzocampo@yahoo.es', 'Pendiente', '90678809', 43, 48, NULL, 0, 1, 1, 1, '2025-03-23 16:57:08'),
(88, 'Norma Moreira Chaves', '105740788', '50688304982', 'nmoreirach@gmail.com', 'Pendiente', '51898227', 45, 50, NULL, 0, 1, 1, 1, '2025-03-23 16:57:08'),
...
(91, 'Laura Moreno González ', '401870945', '50663604228', 'laumoreno237@gmail.com', 'Pendiente', '512054875', 45, 50, NULL, 0, 1, 1, 1, '2025-03-23 16:57:08'),
...
(102, 'Isabel Cristina Rosales Chavarría ', '108390723', '50688238231', 'isabelrosales2910@hotmail.com', 'Pendiente', '6451914592', 45, 50, NULL, 0, 1, 1, 1, '2025-03-23 16:57:08'),
...
(108, 'Ana Isabel Mora Romero', '10710-0406.', '50689165559', 'anita60mr@gmail.com', 'Pendiente', '503773797', 19, 21, NULL, 0, 4, 1, 1, '2025-03-23 16:57:08'),
(109, 'Katherine Siles Madrigal', '110590914', '50683119972', 'kathysilesm20@gmail.com', 'Pendiente', '96488305', 16, 15, NULL, 0, 1, 1, 1, '2025-03-23 16:57:08'),
(110, 'Karen García Quirós ', '118720579', '50672416747', 'karengarciaquiross@gmail.com', 'Pendiente', '44432911', 16, 15, NULL, 0, 1, 1, 1, '2025-03-23 16:57:08'),
...
(112, 'Dayanna García Sánchez ', '0117750879', '50688920041', 'calicalderon09@gmail.com', 'Pendiente', '57799713', 23, 25, NULL, 0, 5, 1, 1, '2025-03-23 16:57:08'),
...
(118, 'Hellen Alvarado Mora ', '11005230', '50660294024', 'hellenalvarado.528@gmail.com', 'Pendiente', '53253310 ', 45, 50, NULL, 0, 1, 1, 1, '2025-03-23 16:57:08'),
...
(121, 'Xinia Arguedas Vega ', '2 462 507', '50671435344', 'xiniarguedas45@gmail.com', 'Pendiente', 'Becada ', 15, 14, NULL, 0, 2, 1, 1, '2025-03-23 16:57:08'),
...
(123, 'Wendy Lorena raudales moncada ', '801270603 ', '50685765304', 'wendyraudalesm@gmail.com', 'Pendiente', '12842178', 44, 49, NULL, 0, 1, 1, 1, '2025-03-23 16:57:08'),
...
(129, 'Jessica Montero Morales', '106790295', '50660407375', 'jemonteromorales@gmail.com', 'Pendiente', '51611011', 35, 40, NULL, 0, 1, 1, 1, '2025-03-24 17:16:31'),
(130, 'Jessica Montero Morales ', '106790295', '50660407375', 'jemonteromorales@gmail.com', 'Pendiente', '60407375', 37, 42, NULL, 0, 1, 1, 1, '2025-03-24 17:18:14'),
...
(142, 'Maria celeste Mena Sanabria ', '402370245', '50672910923', 'celestems131@gmail.com', 'Pendiente', '410850703', 44, 49, NULL, 0, 1, 1, 1, '2025-03-24 17:12:09'),
...
(148, 'Silvia Vargas Hernández ', '401860689 ', '50685473391', 'mivanpri24@gmail.com', 'Pendiente', '48042814', 37, 42, NULL, 0, 1, 1, 1, '2025-03-25 15:45:43'),
(149, 'Adriana Solano', '114160446', '50683082659', 'adri31sc@gmail.com', 'Pendiente', '92261965', 38, 43, NULL, 0, 5, 1, 1, '2025-03-25 19:31:02'),
(150, 'Pamela Álvarez ', '402390755', '50684206680', 'pamelaalvrz04@gmail.com', 'Pendiente', '6503142603', 16, 15, NULL, 0, 1, 1, 1, '2025-03-26 09:24:48'),
...
(157, 'Ligia María Varela Muñoz ', '107370254 ', '50688057888', 'vivivarela68@gmail.com', 'Pendiente', 'BECADA ', 37, 42, NULL, 0, 1, 1, 1, '2025-03-27 10:54:29'),
(158, 'Guisselle Cambronero León ', '401730092 ', '50686051200', 'gleon18@gmail.com', 'Pendiente', '51046001', 22, 23, NULL, 0, 2, 1, 1, '2025-03-27 13:05:20'),
...
(163, 'Kelly Jexabell Sequeira Areas', '155802404327', '50661295332', 'kelly.s.areas94@gmail.com', 'Pendiente', '410582357', 40, 45, NULL, 0, 1, 1, 1, '2025-03-28 12:44:17'),
(164, 'Josseline Daniela Jiménez Rodríguez', '402480522', '50661039812', 'josseline649@gmail.com', 'Pendiente', '83134596', 42, 47, NULL, 0, 1, 1, 1, '2025-03-28 14:59:36'),
(165, 'Adriana Durán Villalobos ', '402200967', '50670157613', 'duranvillalobosadri@gmail.com', 'Pendiente', '53506395', 44, 49, NULL, 0, 1, 1, 1, '2025-03-28 15:46:37'),
...
(168, 'Grettel Barrios Barrientos ', '114850081', '50662428017', 'grettelbarriosaj@gmail.com', 'Pendiente', '6455282230', 23, 25, NULL, 0, 4, 1, 1, '2025-03-28 18:38:42'),
(169, 'MELANY AGUILAR ARGUELLO', '113620190', '50685643372', 'melanytatiana88@gmail.com', 'Pendiente', '6497157801', 27, 30, NULL, 0, 1, 1, 1, '2025-03-28 19:50:47'),
(170, 'Fiorella Chacón Alpizar ', '402390808', '50663741538', 'fiochacon10@hotmail.com', 'Pendiente', '50932490', 44, 49, NULL, 0, 1, 1, 1, '2025-03-29 09:07:10'),
...
(177, 'Leda Rodríguez Ruíz ', '302600543', '50670151084', 'leroru04@gmail.com', 'Pendiente', '58685902', 27, 31, NULL, 0, 1, 1, 1, '2025-03-31 15:41:45'),
...
(183, 'Wendy Campos Rodríguez', '402140567', '50670568174', 'wen.camp141@gmail.com', 'Pendiente', '51562055', 27, 31, NULL, 0, 1, 1, 1, '2025-03-31 20:56:17'),
...
(186, 'Danna Paola Castillo Villalobos ', '119080280', '50687490111', 'dannacastillov2004@gmail.com', 'Pendiente', '521100045', 21, 22, NULL, 0, 1, 1, 1, '2025-03-31 15:16:42'),
...
(191, 'Lorena Cubillo Lobo ', '106210599', '50688950020', 'lorenacubillo93@gmail', 'Pendiente', '95060547', 42, 47, NULL, 0, 1, 1, 1, '2025-04-01 14:55:40'),
...
(198, 'Glenda Miranda Román', '402250682', '50685634514', 'glenda.mr04@gmail.com', 'Pendiente', '592884793', 61, 67, NULL, 0, 1, 1, 1, '2025-04-02 15:25:09'),
...
(201, 'Tatiana Villafuerte Retana ', '402490573 ', '50660060595', 'tvillafuerte.04@outlook.com', 'Pendiente', '54213568', 19, 21, NULL, 0, 4, 1, 1, '2025-04-03 10:11:32'),
(202, 'Xinia Alfaro Bonilla', '401480881', '50688788428', 'xaxalfaroo@gmail.com', 'Pendiente', '58911343', 28, 32, NULL, 0, 1, 1, 1, '2025-04-03 16:18:06'),
(203, 'María Patricia Conejo Rojas', '204220150', '50689894536', 'patriciaconejo@hotmail.com', 'Pendiente', '366961311', 28, 32, NULL, 0, 1, 1, 1, '2025-04-03 17:53:44'),
...
(207, 'Karina Esquivel Solano ', '110970348', '50685491849', 'kriness@gmail.com', 'Pendiente', '50123719 ', 15, 14, NULL, 0, 1, 1, 1, '2025-04-04 10:28:05'),
...
(209, 'Martha Elena Meléndez Monge', '106700550', '50688206921', 'marthammonge@hotmail.com', 'Pendiente', '6849572170', 40, 45, NULL, 0, 1, 1, 1, '2025-04-04 17:29:32'),
(210, 'Ariela Garita Flores', '120720203', '50685384177', 'floreslucy08@gmail.com', 'Pendiente', '88702386', 27, 31, NULL, 0, 1, 1, 1, '2025-04-04 17:44:13'),
(211, 'Lucía Flores Fonseca', '111260671', '50688323567', 'floreslucy08@gmail.com', 'Pendiente', '88702386', 27, 31, NULL, 0, 1, 1, 1, '2025-04-04 17:45:03'),
(213, 'Deyanira Castillo Chaves', '401390551', '50684657497', 'deyacastillo2@gmail.com', 'Pendiente', 'BECADA', 62, 68, NULL, 0, 1, 1, 1, '2025-04-04 18:06:42'),
...
(216, 'Cecilia Dolores Arauz Robleto', '800460437', '50664790267', 'ceciarauz50@gmail.com', 'Pendiente', '04072881', 15, 14, NULL, 0, 2, 1, 1, '2025-04-04 20:03:36'),
...
(218, 'Shirley Patricia Marín Muñoz ', ' 106780125', '50683713717', 'shmarin13@hotmail.com', 'Pendiente', '90604492', 19, 21, NULL, 0, 4, 1, 1, '2025-04-05 20:00:07'),
...
(221, 'Flor María Cascante Fernández', '104130614', '50688820390', 'fmcasf@gmail.com', 'Pendiente', '2025040610283006328573436', 19, 21, NULL, 0, 5, 1, 1, '2025-04-07 18:00:20'),
...
(235, 'Tracy Montero Mora', '115430513', '50683891276', 'nanim264@gmail.com', 'Pendiente', '55743188', 62, 68, NULL, 0, 1, 1, 1, '2025-04-09 16:06:50'),
...
(241, 'Jeannette Chavarría Campos', '106320197', '50661193507', 'jecacha17@hotmail.com', 'Pendiente', '881902998', 18, 20, NULL, 0, 1, 1, 1, '2025-04-10 17:22:45'),
...
(250, 'Leticia Villalobos Ramírez', '108760100', '50683379728', 'lvillalobos032@hotmail.com', 'Pendiente', '18341739', 44, 49, NULL, 0, 1, 1, 1, '2025-04-14 16:28:17'),
...
(256, 'María Patricia Conejo Rojas', '204220150', '50689894536', 'patriciaconejo@hotmail.com', 'Pendiente', '366961311', 18, 19, NULL, 0, 1, 1, 1, '2025-04-14 21:10:10'),
...
(279, 'Karol Gómez Cortés', '304260599', '50671675305', 'karo881414@gmail.com', 'Pendiente', '6485804663', 61, 66, NULL, 0, 1, 1, 1, '2025-04-23 22:32:00'),
(280, 'María Fernanda Pérez Chavarría', '402130665', '50688010207', 'fernanda23.23@hotmail.com', 'Pendiente', '55433703', 61, 66, NULL, 0, 1, 1, 1, '2025-04-23 22:44:33'),
...
(282, 'Ana Isabel Cruz Cruz', '700550620', '50687123608', 'casmanuel1@yahoo.com', 'Pendiente', '55666383', 18, 19, NULL, 0, 1, 1, 1, '2025-04-23 23:16:17'),
...
(297, 'Gisela Espinoza Avelares', '155819996611', '50661675144', 'giseespinoza94@hotmail.com', 'Pendiente', '10421099', 61, 66, NULL, 0, 1, 1, 1, '2025-04-25 22:37:25'),
...
(299, 'Graciela Carolina Medrano Aburto', 'C03828961', '50672669943', 'medranograciela156@gmail.com', 'Pendiente', '6526741161 ', 61, 66, NULL, 0, 1, 1, 1, '2025-04-28 20:33:12'),
...
(305, 'Yessenia Chacón Rodriguez ', '205270658', '50689357026', 'ychaconcavalier@gmail.com', 'Pendiente', '93259998', 69, 75, NULL, 0, 3, 1, 1, '2025-05-05 12:15:43'),
(306, 'Alba Arce Varela', '401630090', '50688342982', 'albaarce1@hotmail.com', 'Pendiente', '25398785', 27, 31, NULL, 0, 2, 1, 1, '2025-05-06 18:11:27'),
(307, 'Joselin Chen Chaves ', '604470170', '50687239696', 'joselynchen24@gmail.com', 'Pendiente', '95067878', 73, 79, NULL, 0, 5, 1, 1, '2025-05-06 15:21:38'),
(308, 'Tamara Castro Venegas', '118680488', '50686519595', 'tamyg.castro@gmail.com', 'Pendiente', '51133406', 18, 19, NULL, 0, 1, 1, 1, '2025-05-07 16:42:20'),
(309, 'Sara campos Rodríguez ', '402730566', '50684597934', 'saracamposrodriguez0880@gmail.com', 'Pendiente', '2025050710283006510341809', 72, 78, NULL, 0, 6, 1, 1, '2025-05-07 18:22:07'),
(311, 'Carolina Gutiérrez Fernández', '401330434', '50683341558', 'carolgdr@hotmail.com', 'Pendiente', '', 28, 32, NULL, 0, 3, 1, 1, '2025-05-08 17:50:19'),
(312, 'Carmen Campos Solís ', ' 203100672', '50687070035', 'carmenemilia56@yahoo.com', 'Pendiente', '17512231', 55, 61, NULL, 0, 3, 1, 1, '2025-05-08 11:55:20'),
(313, 'Yensy Meza Cortés', '303400924', '50683083268', 'yensymeza22@gmail.com', 'Pendiente', '', 26, 29, NULL, 0, 3, 1, 1, '2025-05-12 18:26:38'),
(315, '1231233123123', '12312312312', '50612312312', '123123123123', 'Pendiente', '123123123', 79, 83, NULL, 0, 3, 1, 1, '2025-05-12 20:45:41');
...
(67, 1, 15000.00, 16, 38, 'Decoración con globos - Nivel Básico ', 'CLIENTE', 'SISTEMA', '402070781', 'Marianela Cerdas Gómez ', '50685501226', 'nelac0809@gmail.com', '', '94017511', '2025-03-07 16:33:16', 1, 0, NULL),
(68, 1, 15000.00, 17, 41, 'Decoración con Globos - Nivel Avanzado ', 'CLIENTE', 'SISTEMA', '108410277', 'Miriam Salas Arce ', '50672134027', 'mirsalasa@gmail.com', '', '93986425', '2025-03-07 16:36:26', 1, 0, NULL),
(69, 1, 15000.00, 16, 42, 'Decoración con globos - Nivel Básico ', 'CLIENTE', 'SISTEMA', '207450030', 'Viviana Cerdas Gómez ', '50689778594', 'vivianacerdasgomez@gmail.com', '', '2025030715283006429537559', '2025-03-07 18:02:51', 1, 0, NULL),
...
(278, 1, 15000.00, 61, 247, 'Maquillaje profesional nivel 2', 'CLIENTE', 'SISTEMA', '402340709', 'Zaray Chavez Ocampo', '50672609955', 'zaraychavesocampo@gmail.com', '2025032510283006250390701 el 25/03/2025 banco BAC', '2025032510283006250390701 el 25/03/2025 banco BAC', '2025-04-23 20:43:32', 0, 0, NULL),
(279, 1, 15000.00, 64, 259, 'Manicure profesional - solamente quienes llevaron los 3 primeros meses - Grupo miércoles de 10:30 am', 'CLIENTE', 'SISTEMA', '114730485', 'Paola Solís Meza', '50662970706', 'jenepao460@gmail.com', '93990061 EL 15/04/2025 BN', '93990061', '2025-04-23 21:25:08', 0, 0, NULL),
(280, 1, 15000.00, 18, 263, 'Costura y patronaje desde cero', 'CLIENTE', 'SISTEMA', '108320445', 'María Delgado Calderón', '50672605455', 'maria.delg.7389@gmail.com', '20250416102183006386374575 el 16/04/2025 BAC', '20250416102183006386374575', '2025-04-23 21:33:21', 0, 0, NULL),
(281, 1, 15000.00, 64, 276, 'Manicure profesional - solamente quienes llevaron los 3 primeros meses - Grupo miércoles de 10:30 am', 'CLIENTE', 'SISTEMA', '402190103', 'Catalina Campos Cortés', '50661127211', 'catalinacampos066@gmai.com', '94439633 EL 23/04/2025 BN', '94439633', '2025-04-23 21:57:48', 0, 0, NULL),
(282, 1, 15000.00, 64, 277, 'Manicure profesional - solamente quienes llevaron los 3 primeros meses - Grupo miércoles de 10:30 am', 'CLIENTE', 'SISTEMA', '106360220', 'Ana Isabel Reyes Herra', '50683825341', 'anareyesherra@outlook.com', '2025041410283006374834194 POR 14 MIL Y 2025041410283006376453311 POR MIL EL 14/04/2025', '76453311 Y 74834191', '2025-04-23 22:16:04', 0, 0, NULL),
(283, 1, 3750.00, 67, 278, 'Corte básico y alta peluquería', 'CLIENTE', 'SISTEMA', '701670581', 'Yendry Díaz Valverde', '50663562510', 'yendrydiazv@gmail.com', '52353242 EL 23/04/2025 BCR', '52353242', '2025-04-23 22:24:37', 0, 0, NULL),
(284, 1, 15000.00, 61, 279, 'Maquillaje profesional nivel 2', 'CLIENTE', 'SISTEMA', '304260599', 'Karol Gómez Cortés', '50671675305', 'karo881414@gmail.com', '2025042310283006422305164 el 23/04/2025 BAC', '2025042310283006422305164', '2025-04-23 22:32:43', 0, 0, NULL),
(285, 1, 15000.00, 61, 280, 'Maquillaje profesional nivel 2', 'CLIENTE', 'SISTEMA', '402130665', 'María Fernanda Pérez Chavarría', '50688010207', 'fernanda23.23@hotmail.com', '2025040110283006300900811 el 01/04/2025 BAC', '2025040110283006300900811', '2025-04-23 22:45:23', 0, 0, NULL),
(286, 1, 15000.00, 66, 281, 'Costura básica desde cero - nivel intensivo', 'CLIENTE', 'SISTEMA', '114320172', 'Yareliz Alvardao Cambronero', '50686504693', 'yarelizac@hotmail.com', '52626964 el 23/04/2025 BCR', '52626964', '2025-04-23 23:01:45', 0, 0, NULL),
(287, 1, 15000.00, 65, 283, 'Manicure profesional - solamente quienes llevaron los 3 primeros meses - Grupo viernes de 10:30 am a', 'CLIENTE', 'SISTEMA', '401970615', 'Dayana Alfaro Esquivel', '50672032960', 'daalfaroe@gmail.com', '95361436 el 24/04/2025 BN', '95361436', '2025-04-24 16:41:28', 0, 0, NULL),
(288, 1, 15000.00, 18, 282, 'Costura y patronaje desde cero', 'CLIENTE', 'SISTEMA', '700550620', 'Ana Isabel Cruz Cruz', '50687123608', 'casmanuel1@yahoo.com', 'FT25113M4B EL 23/04/2025 BANCO POPULAR', 'FT25113M4B', '2025-04-24 18:38:58', 0, 0, NULL),
(289, 1, 15000.00, 65, 284, 'Manicure profesional - solamente quienes llevaron los 3 primeros meses - Grupo viernes de 10:30 am a', 'CLIENTE', 'SISTEMA', '402750635', 'Josebeth Arrieta Zúñiga', '50686084624', 'jojoarixx@gmail.com', '95663994 el 24/04/2025 BN', '95663994', '2025-04-24 20:14:51', 0, 0, NULL),
(290, 1, 15000.00, 38, 285, 'Portugués - principiantes (virtual)', 'CLIENTE', 'SISTEMA', '402360387', 'Cindy Gutiérrez Abarca', '50689525072', 'gutierreza.facturas@gmail.com', '2025042510283006430123177 el 25/04/2025 BAC', '2025042510283006430123177', '2025-04-25 14:14:09', 0, 0, NULL),
(291, 1, 15000.00, 65, 258, 'Manicure profesional - solamente quienes llevaron los 3 primeros meses - Grupo viernes de 10:30 am a', 'CLIENTE', 'SISTEMA', '118080544', 'Nicole Brenes Solís', '50684130962', 'nicolbrso1204@gmail.com', '56418146 el 14/04/2025 BN', '56418146', '2025-04-25 14:21:50', 0, 0, NULL),
(292, 1, 15000.00, 65, 261, 'Manicure profesional - solamente quienes llevaron los 3 primeros meses - Grupo viernes de 10:30 am a', 'CLIENTE', 'SISTEMA', '117850083', 'Nicole Melissa Solís Salgado', '50664723689', 'melissasolis0770@gmail.com', '52987285 el 15/04/2025 SCOTIABANK', '52987285', '2025-04-25 14:24:39', 0, 0, NULL),
(293, 1, 15000.00, 65, 262, 'Manicure profesional - solamente quienes llevaron los 3 primeros meses - Grupo viernes de 10:30 am a', 'CLIENTE', 'SISTEMA', '402020212', 'Daniela María Oconitrillo Garita', '50688194962', 'daniocog@gmail.com', ' 50003691 el 15/04/2025 BCR', '50003691', '2025-04-25 14:27:08', 0, 0, NULL),
(294, 1, 15000.00, 63, 286, 'Manicure profesional - solamente quienes llevaron los 3 primeros meses - Grupo martes 7 pm a 9 pm', 'CLIENTE', 'SISTEMA', '111030051', 'Verónica Guevara Corrales ', '50688272555', 'veronik29@yahoo.com', '2025042416183220572360334 el 24/04/2025 BANCO POPULAR', '2025042416183220572360334', '2025-04-25 14:37:59', 0, 0, NULL),
(295, 1, 15000.00, 65, 287, 'Manicure profesional - solamente quienes llevaron los 3 primeros meses - Grupo viernes de 10:30 am a', 'CLIENTE', 'SISTEMA', '110410474', 'Rose Miranda González', '50687058435', 'odontomundo@yahoo.com', '2025042410283006428599041 el 24/04/2025 BAC', '2025042410283006428599041', '2025-04-25 14:51:19', 0, 0, NULL),
(296, 1, 15000.00, 22, 288, 'Jabonería artesanal', 'CLIENTE', 'SISTEMA', '108080542', 'Gabriela Arce Rubí', '50689229987', 'garce0211@gmail.com', '96274494 el 24/04/2025 BN', '96274494', '2025-04-25 14:56:25', 0, 0, NULL),
(297, 1, 15000.00, 31, 289, 'Quilting', 'CLIENTE', 'SISTEMA', '104630531', 'Yamileth Ruíz Vargas', '50688606307', 'yami@transportes-chaves.com', '2025042415283006760598725 el 25/04/2025 BN', '2025042415283006760598725', '2025-04-25 16:11:20', 0, 0, NULL),
(298, 1, 15000.00, 65, 290, 'Manicure profesional - solamente quienes llevaron los 3 primeros meses - Grupo viernes de 10:30 am a', 'CLIENTE', 'SISTEMA', '116000228', 'María Fernanda Alvarado Jiménez', '50684503385', 'andreysolis8916@gmail.com', '96817312 el 25/04/2025 BN', '96817312 ', '2025-04-25 17:01:08', 0, 0, NULL),
(299, 1, 15000.00, 60, 194, 'Masaje terapéutico - nivel avanzado', 'CLIENTE', 'SISTEMA', '4-0180-0485 ', 'Milena Espinoza Rojas ', '50660565826', 'milenaer12@gmail.com', 'BECADA', 'BECADA', '2025-04-25 18:04:37', 0, 0, NULL),
(300, 1, 15000.00, 62, 291, 'Alambrismo y Bisutería - Nivel Intermedio', 'CLIENTE', 'SISTEMA', '401080393', 'María de los Ángeles Miranda Palma', '50688104611', 'mirandapalmamarielos@gmail.com', '99529242 el 12/04/2025 BN', '99529242', '2025-04-25 18:17:39', 0, 0, NULL),
(301, 1, 15000.00, 63, 260, 'Manicure profesional - solamente quienes llevaron los 3 primeros meses - Grupo martes 7 pm a 9 pm', 'CLIENTE', 'SISTEMA', '401990430', 'Susana Segura Cortés', '50688365492', 'susansc45@gmail.com', '94356295 EL 15/04/2025 BN', '94356295', '2025-04-25 18:22:29', 0, 0, NULL),
(302, 1, 15000.00, 64, 292, 'Manicure profesional - solamente quienes llevaron los 3 primeros meses - Grupo miércoles de 10:30 am', 'CLIENTE', 'SISTEMA', '402300619', 'Mariana Sánchez Paniagua', '50660049256', 'marisanchez-1996@hotmail.com', '63828454 el 25/04/2025 BCR', '63828454', '2025-04-25 20:38:06', 0, 0, NULL),
(303, 1, 15000.00, 70, 295, 'Corte básico y alta peluquería', 'CLIENTE', 'SISTEMA', '701670581', 'Yendry Díaz Valverde', '50663562510', 'yendrydiazv@gmail.com', '52353242 el 23/04/2025 BCR', '52353242', '2025-04-25 22:20:40', 0, 0, NULL),
(304, 1, 15000.00, 70, 294, 'Corte básico y alta peluquería', 'CLIENTE', 'SISTEMA', '402550825', 'Abril Gabriela Vindas Castro', '50672868191', 'avindas2014gmail.com', '92358424 el 22/04/2025 BN', '92358421', '2025-04-25 22:21:25', 0, 0, NULL),
(305, 1, 15000.00, 61, 297, 'Maquillaje profesional nivel 2', 'CLIENTE', 'SISTEMA', '155819996611', 'Gisela Espinoza Avelares', '50661675144', 'giseespinoza94@hotmail.com', '97443556 el 25/04/2025 BN', '97443556', '2025-04-25 22:38:03', 0, 0, NULL),
(306, 1, 15000.00, 61, 296, 'Maquillaje profesional nivel 2', 'CLIENTE', 'SISTEMA', '119950153', 'María Luisa Acosta Zárate', '50660142698', 'malua2007@gmail.com', '2025042510283006433666079 EL 25/04/2025 BN', '2025042510283006433666079', '2025-04-25 22:38:56', 0, 0, NULL),
(307, 1, 15000.00, 61, 298, 'Maquillaje profesional nivel 2', 'CLIENTE', 'SISTEMA', '401560689', 'Lourdes María Arce Espinoza', '50683043666', 'arcelourdes1@gmail.com', '97514544 el 25/04/2025 BN', '97514544', '2025-04-25 23:14:51', 0, 0, NULL),
(308, 1, 15000.00, 61, 299, 'Maquillaje profesional nivel 2', 'CLIENTE', 'SISTEMA', 'C03828961', 'Graciela Carolina Medrano Aburto', '50672669943', 'medranograciela156@gmail.com', '2025042810283006447736860 el 28/04/2025 BAC', '2025042810283006447736860', '2025-04-28 20:34:23', 0, 0, NULL),
(309, 1, 23000.00, 71, 300, 'Inglés - profe Jacqueline Guzmán', 'CLIENTE', 'SISTEMA', '401320083', 'María de los Ángeles Hernández Hernández', '50685461917', 'marieloshernandez086@gmail.com', 'BECADA', 'BECADA', '2025-04-28 22:35:36', 0, 0, NULL),
(310, 1, 23000.00, 71, 301, 'Inglés - profe Jacqueline Guzmán', 'CLIENTE', 'SISTEMA', '109590444', 'Ana Jiménez Arroyo', '50688897595', 'infogrupoakara@gmail.com', '2025042810283006453321436 el 28/04/2025 BAC', '2025042810283006453321436', '2025-04-29 14:56:57', 0, 0, NULL),
(311, 1, 15000.00, 70, 302, 'Corte básico y alta peluquería', 'CLIENTE', 'SISTEMA', '401740279', 'Jacqueline Gutiérrez Duarte', '50670196887', 'jackeline.g.d.81qgmail.com', '88950544 el 29/04/2025 BCR', '88950544', '2025-04-29 15:12:38', 0, 0, NULL),
(312, 1, 15000.00, 61, 303, 'Maquillaje profesional nivel 2', 'CLIENTE', 'SISTEMA', '402800481', 'Samantha Araya Jiménez', '50663771187', 'saudyjisa87@otlook.es', '88369722 el 28/04/2025 BCR', '88369722', '2025-04-29 16:30:50', 0, 0, NULL),
(313, 1, 15000.00, 44, 64, 'Repostería', 'CLIENTE', 'SISTEMA', '303750678', 'Vanessa Rodríguez Serrano ', '50661481859', 'vrs3981@hotmail.com', 'BECADA', 'BECADA', '2025-04-29 17:48:23', 0, 0, NULL),
(314, 1, 15000.00, 38, 304, 'Portugués - principiantes (virtual)', 'CLIENTE', 'SISTEMA', '604470170', 'Joselin Chen Chaves ', '50687239696', 'joselynchen24@gmail.com', '98837537 EL 02/05/2025', '98837537', '2025-05-02 21:10:52', 0, 0, NULL),
(315, 1, 15000.00, 18, 308, 'Costura y patronaje desde cero', 'CLIENTE', 'SISTEMA', '118680488', 'Tamara Castro Venegas', '50686519595', 'tamyg.castro@gmail.com', '51133406 el 07/05/2025', '51133406', '2025-05-07 16:42:57', 0, 0, NULL),
(316, 1, 15000.00, 73, 307, 'Inglés - modalidad virtual', 'CLIENTE', 'SISTEMA', '604470170', 'Joselin Chen Chaves ', '50687239696', 'joselynchen24@gmail.com', '95067878 el 07/05/2025', '95067878', '2025-05-07 16:44:17', 0, 0, NULL),
(317, 1, 15000.00, 15, 207, 'Almohadones, cojines y más – Nivel intermedio - avanzado', 'CLIENTE', 'SISTEMA', '110970348', 'Karina Esquivel Solano ', '50685491849', 'kriness@gmail.com', '50126719 el 05/05/2025 BN', '50123719 ', '2025-05-12 21:02:21', 0, 0, NULL),
(318, 1, 15000.00, 16, 38, 'Decoración con globos - Nivel Básico ', 'CLIENTE', 'SISTEMA', '402070781', 'Marianela Cerdas Gómez ', '50685501226', 'nelac0809@gmail.com', '6526881 EL 06/05/2025 BN', '6526881', '2025-05-12 21:04:13', 0, 0, NULL),
(319, 1, 15000.00, 16, 62, 'Decoración con globos - Nivel Básico ', 'CLIENTE', 'SISTEMA', '108270565 ', 'Patricia Ramírez Rimolo ', '50670395794', 'masiescara98@gmail.com', '6495351514 05/05/2025 BAC', '6495351514', '2025-05-12 21:13:23', 0, 0, NULL),
(320, 1, 15000.00, 16, 109, 'Decoración con globos - Nivel Básico ', 'CLIENTE', 'SISTEMA', '110590914', 'Katherine Siles Madrigal', '50683119972', 'kathysilesm20@gmail.com', '96488305 07/05/2025 BAC', '96488305', '2025-05-12 21:14:19', 0, 0, NULL),
(321, 1, 15000.00, 16, 110, 'Decoración con globos - Nivel Básico ', 'CLIENTE', 'SISTEMA', '118720579', 'Karen García Quirós ', '50672416747', 'karengarciaquiross@gmail.com', '44432911 06/05/2025 BCR', '44432911', '2025-05-12 21:15:06', 0, 0, NULL),
(322, 1, 15000.00, 16, 150, 'Decoración con globos - Nivel Básico ', 'CLIENTE', 'SISTEMA', '402390755', 'Pamela Álvarez ', '50684206680', 'pamelaalvrz04@gmail.com', '6503142603 06/05/2025 BAC', '6503142603', '2025-05-12 21:15:53', 0, 0, NULL),
(323, 1, 15000.00, 17, 13, 'Decoración con Globos - Nivel Avanzado ', 'CLIENTE', 'SISTEMA', '20482781', 'Carmen Murillo Espinoza', '50689873903', 'carmenmurilloespinoza@gmail.com', '54713110 07/05/2025 BN', '54713110', '2025-05-12 21:16:52', 0, 0, NULL),
(324, 1, 15000.00, 17, 14, 'Decoración con Globos - Nivel Avanzado ', 'CLIENTE', 'SISTEMA', '2473966', 'Guadalupe Murillo Espinoza', '50685068415', 'murillog972@gmail.com', '54713110 07/05/2025 BN', '54713110', '2025-05-12 21:17:26', 0, 0, NULL),
(325, 1, 15000.00, 17, 41, 'Decoración con Globos - Nivel Avanzado ', 'CLIENTE', 'SISTEMA', '108410277', 'Miriam Salas Arce ', '50672134027', 'mirsalasa@gmail.com', '56520979 08/05/2025 BN', '56520979 ', '2025-05-12 21:18:10', 0, 0, NULL),
(326, 1, 15000.00, 17, 57, 'Decoración con Globos - Nivel Avanzado ', 'CLIENTE', 'SISTEMA', '109600558', 'Paola Vargas', '506', 'jpavazu@gmail.com', 'BECADA', 'BECADA', '2025-05-12 21:34:29', 0, 0, NULL),
(327, 1, 15000.00, 18, 241, 'Costura y patronaje desde cero', 'CLIENTE', 'SISTEMA', '106320197', 'Jeannette Chavarría Campos', '50661193507', 'jecacha17@hotmail.com', '881902998 11/05/2025 BN', '881902998', '2025-05-12 21:35:15', 0, 0, NULL),
(328, 1, 15000.00, 18, 256, 'Costura y patronaje desde cero', 'CLIENTE', 'SISTEMA', '204220150', 'María Patricia Conejo Rojas', '50689894536', 'patriciaconejo@hotmail.com', '366961311 02/05/2025 CAJA DE ANDE', '366961311', '2025-05-12 21:36:15', 0, 0, NULL),
(329, 1, 15000.00, 28, 203, 'Bordado a mano', 'CLIENTE', 'SISTEMA', '204220150', 'María Patricia Conejo Rojas', '50689894536', 'patriciaconejo@hotmail.com', '366961311 02/05/2025 CAJA DE ANDE', '366961311', '2025-05-12 21:36:35', 0, 0, NULL),
(330, 1, 15000.00, 18, 282, 'Costura y patronaje desde cero', 'CLIENTE', 'SISTEMA', '700550620', 'Ana Isabel Cruz Cruz', '50687123608', 'casmanuel1@yahoo.com', '55666383 08/05/2025 BN', '55666383', '2025-05-12 21:37:49', 0, 0, NULL),
(331, 1, 15000.00, 18, 308, 'Costura y patronaje desde cero', 'CLIENTE', 'SISTEMA', '118680488', 'Tamara Castro Venegas', '50686519595', 'tamyg.castro@gmail.com', '51133406 07/05/2025 BN', '51133406', '2025-05-12 21:38:30', 0, 0, NULL),
(332, 1, 15000.00, 19, 108, 'Introducción a la cosmética natural', 'CLIENTE', 'SISTEMA', '10710-0406.', 'Ana Isabel Mora Romero', '50689165559', 'anita60mr@gmail.com', '503773797 06/05/2025 BN', '503773797', '2025-05-12 21:39:28', 0, 0, NULL),
(333, 1, 15000.00, 19, 201, 'Introducción a la cosmética natural', 'CLIENTE', 'SISTEMA', '402490573 ', 'Tatiana Villafuerte Retana ', '50660060595', 'tvillafuerte.04@outlook.com', '54213568 30/04/2025 BAC', '54213568', '2025-05-12 21:42:58', 0, 0, NULL),
(334, 1, 15000.00, 19, 218, 'Introducción a la cosmética natural', 'CLIENTE', 'SISTEMA', ' 106780125', 'Shirley Patricia Marín Muñoz ', '50683713717', 'shmarin13@hotmail.com', '12079860 04/05/2025 BN', '90604492', '2025-05-12 21:43:56', 0, 0, NULL),
(335, 1, 15000.00, 21, 186, 'Cosmética natural – avanzado (solamente quienes llevaron los 3 meses, enero a marzo 2025)', 'CLIENTE', 'SISTEMA', '119080280', 'Danna Paola Castillo Villalobos ', '50687490111', 'dannacastillov2004@gmail.com', '521100045 09/05/2025 BAC', '521100045', '2025-05-12 21:45:13', 0, 0, NULL),
(336, 1, 15000.00, 23, 65, 'Maquillaje profesional ', 'CLIENTE', 'SISTEMA', '112860687', 'Kerlyn Fernández Obando ', '50683193219', 'kerlyn_07@yahoo.es', '47439969 06/05/2025 BCR', '47439969', '2025-05-12 21:59:56', 0, 0, NULL),
(337, 1, 15000.00, 23, 75, 'Maquillaje profesional ', 'CLIENTE', 'SISTEMA', '112780769', 'Doris Carolina Rodríguez Ramírez', '50683467785', 'dorys18@gmail.com', '66633261 03/05/2025 BN', '66633261', '2025-05-12 22:00:28', 0, 0, NULL),
(338, 1, 15000.00, 23, 168, 'Maquillaje profesional ', 'CLIENTE', 'SISTEMA', '114850081', 'Grettel Barrios Barrientos ', '50662428017', 'grettelbarriosaj@gmail.com', '6455282230 29/04/2025 BAC', '6455282230', '2025-05-12 22:01:18', 0, 0, NULL),
(339, 1, 15000.00, 61, 279, 'Maquillaje profesional nivel 2', 'CLIENTE', 'SISTEMA', '304260599', 'Karol Gómez Cortés', '50671675305', 'karo881414@gmail.com', '6485804663 03/05/2025 BAC', '6485804663', '2025-05-12 22:02:11', 0, 0, NULL),
(340, 1, 15000.00, 61, 280, 'Maquillaje profesional nivel 2', 'CLIENTE', 'SISTEMA', '402130665', 'María Fernanda Pérez Chavarría', '50688010207', 'fernanda23.23@hotmail.com', '55433703 05/05/2025 BN', '55433703', '2025-05-12 22:03:07', 0, 0, NULL),
(341, 1, 15000.00, 61, 297, 'Maquillaje profesional nivel 2', 'CLIENTE', 'SISTEMA', '155819996611', 'Gisela Espinoza Avelares', '50661675144', 'giseespinoza94@hotmail.com', '10421099 10/05/2025 BN', '10421099', '2025-05-12 22:04:01', 0, 0, NULL),
(342, 1, 15000.00, 61, 299, 'Maquillaje profesional nivel 2', 'CLIENTE', 'SISTEMA', 'C03828961', 'Graciela Carolina Medrano Aburto', '50672669943', 'medranograciela156@gmail.com', '6526741161 10/05/2025 BAC', '6526741161 ', '2025-05-12 22:04:47', 0, 0, NULL),
(343, 1, 15000.00, 61, 198, 'Maquillaje profesional nivel 2', 'CLIENTE', 'SISTEMA', '402250682', 'Glenda Miranda Román', '50685634514', 'glenda.mr04@gmail.com', '592884793 06/05/2025 POPULAR', '592884793', '2025-05-12 22:05:42', 0, 0, NULL),
(344, 1, 15000.00, 27, 53, 'Costura básica desde cero - nivel básico', 'CLIENTE', 'SISTEMA', '1-0621-0927 ', 'Marjorie Bonilla Benavides ', '50683496850', 'yuriboni64@gmail.com', '366234766 30/04/2025 CAJA DE ANDE', '366234766', '2025-05-12 22:08:36', 0, 0, NULL),
(345, 1, 15000.00, 27, 54, 'Costura básica desde cero - nivel básico', 'CLIENTE', 'SISTEMA', '104520944', 'Elizabeth Vargas López ', '50661148230', 'marieliza317@gmail.com', '579041865 28/04/2025 POPULAR', '579041865', '2025-05-12 22:09:14', 0, 0, NULL),
(346, 1, 15000.00, 27, 169, 'Costura básica desde cero - nivel básico', 'CLIENTE', 'SISTEMA', '113620190', 'MELANY AGUILAR ARGUELLO', '50685643372', 'melanytatiana88@gmail.com', '6497157801 05/05/2025 BAC', '6497157801', '2025-05-12 22:09:54', 0, 0, NULL),
(347, 1, 15000.00, 27, 177, 'Costura básica desde cero - nivel básico', 'CLIENTE', 'SISTEMA', '302600543', 'Leda Rodríguez Ruíz ', '50670151084', 'leroru04@gmail.com', '58685902 05/05/2025 BN', '58685902', '2025-05-12 22:10:35', 0, 0, NULL),
(348, 1, 15000.00, 27, 183, 'Costura básica desde cero - nivel básico', 'CLIENTE', 'SISTEMA', '402140567', 'Wendy Campos Rodríguez', '50670568174', 'wen.camp141@gmail.com', '51562055 05/05/2025 POPULAR', '51562055', '2025-05-12 22:11:13', 0, 0, NULL),
(349, 1, 15000.00, 27, 210, 'Costura básica desde cero - nivel básico', 'CLIENTE', 'SISTEMA', '120720203', 'Ariela Garita Flores', '50685384177', 'floreslucy08@gmail.com', '88702386 28/04/2025 BCR', '88702386', '2025-05-12 22:11:59', 0, 0, NULL),
(350, 1, 15000.00, 27, 211, 'Costura básica desde cero - nivel básico', 'CLIENTE', 'SISTEMA', '111260671', 'Lucía Flores Fonseca', '50688323567', 'floreslucy08@gmail.com', '88702386 28/04/2025 BCR', '88702386', '2025-05-12 22:12:38', 0, 0, NULL),
(351, 1, 15000.00, 27, 306, 'Costura básica desde cero - nivel básico', 'CLIENTE', 'SISTEMA', '401630090', 'Alba Arce Varela', '50688342982', 'albaarce1@hotmail.com', '25398785 03/05/2025 BCR', '25398785', '2025-05-12 22:13:25', 0, 0, NULL),
(352, 1, 15000.00, 28, 202, 'Bordado a mano', 'CLIENTE', 'SISTEMA', '401480881', 'Xinia Alfaro Bonilla', '50688788428', 'xaxalfaroo@gmail.com', '58911343 08/05/2025 POPULAR', '58911343', '2025-05-12 22:14:13', 0, 0, NULL),
(353, 1, 15000.00, 35, 129, 'Canva - principiantes (virtual)', 'CLIENTE', 'SISTEMA', '106790295', 'Jessica Montero Morales', '50660407375', 'jemonteromorales@gmail.com', '51611011 05/05/2025 BN', '51611011', '2025-05-12 22:15:03', 0, 0, NULL),
(354, 1, 15000.00, 37, 130, 'Creación de contenido para redes sociales', 'CLIENTE', 'SISTEMA', '106790295', 'Jessica Montero Morales ', '50660407375', 'jemonteromorales@gmail.com', '60407375 05/05/2025 popular', '60407375', '2025-05-12 22:17:01', 0, 0, NULL),
(355, 1, 15000.00, 37, 148, 'Creación de contenido para redes sociales', 'CLIENTE', 'SISTEMA', '401860689 ', 'Silvia Vargas Hernández ', '50685473391', 'mivanpri24@gmail.com', '48042814 6/05/2025 BCR', '48042814', '2025-05-12 22:17:44', 0, 0, NULL),
(356, 1, 15000.00, 37, 157, 'Creación de contenido para redes sociales', 'CLIENTE', 'SISTEMA', '107370254 ', 'Ligia María Varela Muñoz ', '50688057888', 'vivivarela68@gmail.com', 'BECADA', 'BECADA ', '2025-05-12 22:18:12', 0, 0, NULL),
(357, 1, 15000.00, 40, 37, 'Alambrismo y Bisutería - Nivel Principiante', 'CLIENTE', 'SISTEMA', '402730546', 'Valentina gabriela Morales Sánchez ', '50684490254', 'valentinagabriela1406@gmail.com', '46064421 06/05/2025 BCR', '46064421', '2025-05-12 22:19:26', 0, 0, NULL),
(358, 1, 15000.00, 40, 163, 'Alambrismo y Bisutería - Nivel Principiante', 'CLIENTE', 'SISTEMA', '155802404327', 'Kelly Jexabell Sequeira Areas', '50661295332', 'kelly.s.areas94@gmail.com', '410582357 04/05/2025 DAVIVIENDA', '410582357', '2025-05-12 22:20:14', 0, 0, NULL),
(359, 1, 15000.00, 40, 209, 'Alambrismo y Bisutería - Nivel Principiante', 'CLIENTE', 'SISTEMA', '106700550', 'Martha Elena Meléndez Monge', '50688206921', 'marthammonge@hotmail.com', '6849572170 06/05/2025 BN', '6849572170', '2025-05-12 22:21:03', 0, 0, NULL),
(360, 1, 15000.00, 62, 213, 'Alambrismo y Bisutería - Nivel Intermedio', 'CLIENTE', 'SISTEMA', '401390551', 'Deyanira Castillo Chaves', '50684657497', 'deyacastillo2@gmail.com', 'BECADA', 'BECADA', '2025-05-12 22:21:26', 0, 0, NULL),
(361, 1, 15000.00, 62, 235, 'Alambrismo y Bisutería - Nivel Intermedio', 'CLIENTE', 'SISTEMA', '115430513', 'Tracy Montero Mora', '50683891276', 'nanim264@gmail.com', '55743188 07/05/2025 BCR', '55743188', '2025-05-12 22:22:08', 0, 0, NULL),
(362, 1, 15000.00, 42, 60, 'Panadería', 'CLIENTE', 'SISTEMA', '401940089', 'José Mario Valerio Berrocal ', '50684553578', 'jvalerio419@gmail.com', '95926675 30/04/2025 BN', '95926675 ', '2025-05-12 22:23:06', 0, 0, NULL),
(363, 1, 15000.00, 42, 164, 'Panadería', 'CLIENTE', 'SISTEMA', '402480522', 'Josseline Daniela Jiménez Rodríguez', '50661039812', 'josseline649@gmail.com', '83134596 12/05/2025 BCR', '83134596', '2025-05-12 22:23:45', 0, 0, NULL),
(364, 1, 15000.00, 42, 191, 'Panadería', 'CLIENTE', 'SISTEMA', '106210599', 'Lorena Cubillo Lobo ', '50688950020', 'lorenacubillo93@gmail', '95060547 30/04/2025 BN', '95060547', '2025-05-12 22:25:05', 0, 0, NULL),
(365, 1, 15000.00, 43, 20, 'Pastelería – nivel #1', 'CLIENTE', 'SISTEMA', '206690166', 'Catalina Sánchez Sánchez ', '50683345869', 'cata23ss@hotmail.com', '93929170 05/05/2025 BN', '93929170', '2025-05-12 22:25:48', 0, 0, NULL),
(366, 1, 15000.00, 43, 73, 'Pastelería – nivel #1', 'CLIENTE', 'SISTEMA', '402210923', 'Valeria campos lobo', '50672041477', 'valecl2693@gmail.com', '9189733 07/05/2025 BAC', '9189733', '2025-05-12 22:26:34', 0, 0, NULL),
(367, 1, 15000.00, 43, 84, 'Pastelería – nivel #1', 'CLIENTE', 'SISTEMA', '800770707', 'Mercedes Manzanares Barahona', '50687922465', 'william.Bermudez.ramirez@gmail.com', '92865631 12/05/2025 BN', '92865631', '2025-05-12 22:27:15', 0, 0, NULL),
(368, 1, 15000.00, 43, 87, 'Pastelería – nivel #1', 'CLIENTE', 'SISTEMA', '108080245', 'Marycruz Ocampo Aguilar ', '50688388581', 'maricruzocampo@yahoo.es', '90678809 05/05/2025 BN', '90678809', '2025-05-12 22:27:51', 0, 0, NULL),
(369, 1, 15000.00, 44, 28, 'Repostería', 'CLIENTE', 'SISTEMA', '701100837', 'Milena Mendoza Valenciano', '50689932919', 'mmendoza@una.ac.cr', '94960341 06/05/2025 BN', '94960341', '2025-05-12 22:28:34', 0, 0, NULL),
(370, 1, 15000.00, 44, 50, 'Repostería', 'CLIENTE', 'SISTEMA', '206690166', 'Catalina Sánchez Sánchez ', '50683345869', 'cata23ss@hotmail.com', '93927078 05/05/2025 BN', '93927078', '2025-05-12 22:29:14', 0, 0, NULL),
(371, 1, 15000.00, 44, 51, 'Repostería', 'CLIENTE', 'SISTEMA', '117640290', 'Krysta Nicole Navarro Carvajal', '50687842662', 'nicolnc02@gmail.com', '51859018 06/05/2025 BN', '52859018', '2025-05-12 22:29:54', 0, 0, NULL),
(372, 1, 15000.00, 44, 52, 'Repostería', 'CLIENTE', 'SISTEMA', '402110502', 'Irene Villalobos León', '50689158623', 'ire_villalobos_18@hotmail.com', '94889657 06/05/2025 BN', '94889657', '2025-05-12 22:30:33', 0, 0, NULL),
(373, 1, 15000.00, 44, 64, 'Repostería', 'CLIENTE', 'SISTEMA', '303750678', 'Vanessa Rodríguez Serrano ', '50661481859', 'vrs3981@hotmail.com', 'BECADA', 'BECADA', '2025-05-12 22:30:56', 0, 0, NULL),
(374, 1, 15000.00, 44, 123, 'Repostería', 'CLIENTE', 'SISTEMA', '801270603 ', 'Wendy Lorena raudales moncada ', '50685765304', 'wendyraudalesm@gmail.com', '12842178 05/05/2025 BN', '12842178', '2025-05-12 22:31:41', 0, 0, NULL),
(375, 1, 15000.00, 44, 142, 'Repostería', 'CLIENTE', 'SISTEMA', '402370245', 'Maria celeste Mena Sanabria ', '50672910923', 'celestems131@gmail.com', '410850703 05/05/2025 DAVIVIENDA', '410850703', '2025-05-12 22:32:24', 0, 0, NULL),
(376, 1, 15000.00, 44, 165, 'Repostería', 'CLIENTE', 'SISTEMA', '402200967', 'Adriana Durán Villalobos ', '50670157613', 'duranvillalobosadri@gmail.com', '53506395 07/05/2025 BN', '53506395', '2025-05-12 22:32:56', 0, 0, NULL),
(377, 1, 15000.00, 44, 170, 'Repostería', 'CLIENTE', 'SISTEMA', '402390808', 'Fiorella Chacón Alpizar ', '50663741538', 'fiochacon10@hotmail.com', '50932490 05/05/2025 POPULAR', '50932490', '2025-05-12 22:33:42', 0, 0, NULL),
(378, 1, 15000.00, 44, 250, 'Repostería', 'CLIENTE', 'SISTEMA', '108760100', 'Leticia Villalobos Ramírez', '50683379728', 'lvillalobos032@hotmail.com', '18341739 02/05/2025 BN', '18341739', '2025-05-12 22:34:25', 0, 0, NULL),
(379, 1, 15000.00, 45, 74, 'Pastelería – nivel #2', 'CLIENTE', 'SISTEMA', '401440460 ', 'Elodia Hernandez Vargas ', '50687830596', 'elo.her@hotmail.es', '6502267521 06/05/2025 BAC', '6502267521', '2025-05-12 22:35:16', 0, 0, NULL),
(380, 1, 15000.00, 45, 88, 'Pastelería – nivel #2', 'CLIENTE', 'SISTEMA', '105740788', 'Norma Moreira Chaves', '50688304982', 'nmoreirach@gmail.com', '51898227 30/04/2025 POPULAR', '51898227', '2025-05-12 22:36:03', 0, 0, NULL),
(381, 1, 15000.00, 45, 91, 'Pastelería – nivel #2', 'CLIENTE', 'SISTEMA', '401870945', 'Laura Moreno González ', '50663604228', 'laumoreno237@gmail.com', '512054875 07/05/2025 BN', '512054875', '2025-05-12 22:37:41', 0, 0, NULL),
(382, 1, 15000.00, 45, 102, 'Pastelería – nivel #2', 'CLIENTE', 'SISTEMA', '108390723', 'Isabel Cristina Rosales Chavarría ', '50688238231', 'isabelrosales2910@hotmail.com', '6451914592 28/04/2025 BAC', '6451914592', '2025-05-12 22:38:32', 0, 0, NULL),
(383, 1, 15000.00, 45, 118, 'Pastelería – nivel #2', 'CLIENTE', 'SISTEMA', '11005230', 'Hellen Alvarado Mora ', '50660294024', 'hellenalvarado.528@gmail.com', '53253310 07/05/2025 BCR', '53253310 ', '2025-05-12 22:39:15', 0, 0, NULL),
(384, 1, 15000.00, 27, 90, 'Costura básica desde cero - nivel básico', 'CLIENTE', 'SISTEMA', '109900292', 'Karol Chinchilla Barrientos', '50687053610', 'karolchiba40@gmail.com', '55804663 05/05/2025 BN', '2025031410283006185431786', '2025-05-12 22:44:19', 0, 0, NULL);
...
(29, 'PROFESOR DE PRUEBA', 'pruebas@email.com', '88888888', '8888888888');
...
(596, 3, 'f2e8eae9b718e6914f8153eab8e69e13', '2025-05-12 23:45:00', '2025-05-13 00:45:00'),
(597, 1, 'ee17da6a0c69cb8e8553a29f6b96cc27', '2025-05-13 02:31:12', '2025-05-13 03:31:12');
...
,`cupo_total` bigint(20) unsigned
,`cantidad_matriculas` bigint(21)
,`MatriculasFueraTiempo` tinyint(1)
);
...
(71, 'Sistema', '50689357026', 'Hola Yessenia Chacón Rodriguez . Hemos recibido su solicitud de matrícula, pronto le informaremos po', '2025-05-05 12:15:44'),
(72, 'Sistema', '50687239696', 'Hola Joselin Chen Chaves . Hemos recibido su solicitud de matrícula, pronto le informaremos por este', '2025-05-06 15:21:40'),
(73, 'Sistema', '50684597934', 'Hola Sara campos Rodríguez . Hemos recibido su solicitud de matrícula, pronto le informaremos por es', '2025-05-07 18:22:09'),
(74, 'Sistema', '50687070035', 'Hola Carmen. Hemos recibido su solicitud de matrícula, pronto le informaremos por este medio cuando ', '2025-05-08 11:45:18'),
(75, 'Sistema', '50687070035', 'Hola Carmen Campos Solís . Hemos recibido su solicitud de matrícula, pronto le informaremos por este', '2025-05-08 11:55:22'),
(76, 'Sistema', '50612312312', 'Hola 123123. Hemos recibido su solicitud de matrícula, pronto le informaremos por este medio cuando ', '2025-05-12 20:38:15'),
(77, 'Sistema', '50612312312', 'Hemos recibido el pago de la cuota. ¡Muchas Gracias! 😊', '2025-05-12 20:41:41'),
(78, 'Sistema', '50612312312', 'Hola 1231233123123. Hemos recibido su solicitud de matrícula, pronto le informaremos por este medio ', '2025-05-12 20:45:42');
...
CREATE ALGORITHM=UNDEFINED DEFINER=`u415516518_agenda_mujeres`@`127.0.0.1` SQL SECURITY DEFINER VIEW `vistacursosactivosv2`  AS SELECT `c`.`ID` AS `IdCurso`, `c`.`NombreCurso` AS `NombreCurso`, `c`.`Precio` AS `Precio`, `c`.`Descripcion` AS `Descripcion`, `c`.`FechaInicio` AS `FechaInicio`, `c`.`FechaFin` AS `FechaFin`, `ci`.`urlImagen` AS `ImagenCurso`, `c`.`cantidadCuotas` AS `cantidadCuotas`, cast(sum(`hc`.`cupo`) as unsigned) AS `cupo_total`, coalesce(`matriculas`.`cantidad_matriculas`,0) AS `cantidad_matriculas`, `c`.`MatriculasFueraTiempo` AS `MatriculasFueraTiempo` FROM (((`cursos` `c` join `horarios_cursos` `hc` on(`c`.`ID` = `hc`.`IdCurso`)) left join `cursoimagen` `ci` on(`c`.`ID` = `ci`.`cursoID`)) left join (select `m`.`IdCurso` AS `IdCurso`,count(`m`.`Id`) AS `cantidad_matriculas` from `matriculas` `m` group by `m`.`IdCurso`) `matriculas` on(`matriculas`.`IdCurso` = `c`.`ID`)) WHERE `c`.`Activo` = '1' GROUP BY `c`.`ID`, `c`.`NombreCurso`, `c`.`Precio`, `c`.`Descripcion`, `c`.`FechaInicio`, `c`.`FechaFin`, `ci`.`urlImagen`, `c`.`cantidadCuotas` ;
...
--
ALTER TABLE `bitacoria_clientes`
  MODIFY `CONSECUTIVO` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=271;
...
--
ALTER TABLE `listapagosprofesor`
  MODIFY `idPago` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=202;
...
--
ALTER TABLE `sesiones`
  MODIFY `IDSesion` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=598;
...
--
ALTER TABLE `WhatsApp_Log`
  MODIFY `idRegistro` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=79;
